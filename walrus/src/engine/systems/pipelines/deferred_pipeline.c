...
ECS_SYSTEM_DECLARE(depth_submit_static_mesh);
ECS_SYSTEM_DECLARE(depth_submit_skinned_mesh);

static void depth_pass(Walrus_FrameGraph *graph, Walrus_FrameNode const *node)
{
    ecs_world_t *ecs = walrus_engine_vars()->ecs;

    Walrus_DeferredRenderer *renderer = walrus_fg_read_ptr(graph, "DeferredRenderer");

    ecs_run(ecs, ecs_id(depth_submit_static_mesh), 0, renderer);
    ecs_run(ecs, ecs_id(depth_submit_skinned_mesh), 0, renderer);
...
static void depth_submit_static_mesh(ecs_iter_t *it)
{
    ...
    for (i32 i = 0; i < it->count; ++i) {
        ...
        walrus_renderer_submit_mesh(renderer, world, meshes[i].mesh, weights);
    }
}

static void depth_submit_skinned_mesh(ecs_iter_t *it)
{
    Walrus_RenderMesh       *meshes   = ecs_field(it, Walrus_RenderMesh, 1);
    Walrus_DeferredRenderer *renderer = it->param;
    ...
    for (i32 i = 0; i < it->count; ++i) {
        ...
        walrus_renderer_submit_skinned_mesh(renderer, p_world, meshes[i].mesh,
                                            ecs_get(it->world, it->entities[i], Walrus_SkinResource)->joint_buffer,
                                            weights);
    }
}
...
Walrus_FramePipeline *walrus_deferred_pipeline_add(Walrus_FrameGraph *graph, char const *name)
{
    ...
    ecs_id(depth_submit_static_mesh) =
        ecs_system(ecs, {
                            .entity             = ecs_entity(ecs, {0}),
                            .query.filter.terms = {{.id = ecs_id(Walrus_RenderMesh)},
                                                   {.id = ecs_id(Walrus_Transform)},
                                                   {.id = ecs_id(Walrus_SkinResource), .oper = EcsNot}},
                            .callback           = depth_submit_static_mesh,
                        });
    ECS_SYSTEM_DEFINE(ecs, depth_submit_skinned_mesh, 0, Walrus_RenderMesh, Walrus_SkinResource);

    Walrus_FramePipeline *deferred_pipeline = walrus_fg_add_pipeline(graph, name);
    /* walrus_fg_add_node(deferred_pipeline, depth_pass, "Depth"); */
